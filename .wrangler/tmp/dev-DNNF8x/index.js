...
var orderItems = sqliteTable("order_items", {
  id: integer("id").primaryKey(),
  // Unique identifier for the order item entry
  ...
  // Price of a single item at the time of purchase
});
...
// src/index.ts
import html from "./1176febf35e4536368ac378a853ec03e22e10dab-frontend.html";
...
var generateUniqueFilename = /* @__PURE__ */ __name((filename) => {
...
app.get("/api/media/images/:key", async (c) => {
...
app.get("/api/media/videos/:key", async (c) => {
  const { key } = c.req.param();
  const object = await c.env.VIDEOS_BUCKET.get(key);
  if (object === null) {
    return c.notFound();
  }
  const headers = new Headers();
...
app.delete("/api/media/:mediaId", authMiddleware, async (c) => {
  ...
  if (!mediaLink) {
    return c.json({ error: "Media link not found" }, 404);
  }
  try {
    await db.delete(productMedia).where(eq(productMedia.id, mediaId));
    const otherLinks = await db.select().from(productMedia).where(eq(productMedia.assetId, mediaLink.assetId)).limit(1);
    if (otherLinks.length === 0) {
      const asset = await db.query.mediaAssets.findFirst({ where: eq(mediaAssets.id, mediaLink.assetId) });
      if (asset) {
        console.log(`Asset ${asset.id} is orphaned. Deleting from R2 and DB.`);
        const bucket = asset.mediaType === "image" ? c.env.IMAGES_BUCKET : c.env.VIDEOS_BUCKET;
        await bucket.delete(asset.r2Key);
        await db.delete(mediaAssets).where(eq(mediaAssets.id, asset.id));
      }
    }
    return c.json({ success: true, message: "Media link deleted successfully" });
  } catch (e) {
    console.error("Failed to delete media:", e);
    return c.json({ error: "Failed to delete media", details: e.message }, 500);
  }
});
app.get("/api/products", async (c) => {
  ...
  const result = await db.query.products.findMany({
    where: category ? eq(products.category, category) : void 0,
    with: {
      media: {
        with: {
          asset: {
  ...
  const fallbackResult = lang !== "en" ? await db.query.products.findMany({
    where: category ? eq(products.category, category) : void 0,
    with: {
      media: {
        with: {
  ...
  const mergedResult = result.map((product) => {
    const fallbackProduct = lang !== "en" ? fallbackResult.find((p) => p.id === product.id) : null;
    const translation = product.translations[0];
    const fallbackTranslation = lang !== "en" ? fallbackProduct?.translations[0] : null;
    const productName = translation?.name || fallbackTranslation?.name || "";
    const productDescription = translation?.description || fallbackTranslation?.description || "";
    return {
      ...product,
      name: productName,
...
app.get("/api/products/categories", async (c) => {
  ...
  return c.json(categories.map((c2) => c2.category));
});
app.get("/api/products/:id", async (c) => {
  ...
  const fallbackTranslation = lang !== "en" ? await db.query.productTranslations.findFirst({
    where: and(
      eq(productTranslations.productId, id),
...
app.post("/api/products/:id/media", authMiddleware, async (c) => {
  ...
  const isProd = c.env.IS_PROD === "true";
  for (const [fieldName, file] of formData.entries()) {
    if (!(file instanceof File) || file.size === 0) continue;
    const mediaType = fieldName.startsWith("image") ? "image" : fieldName.startsWith("video") ? "video" : null;
    if (!mediaType) continue;
    try {
      const fileBuffer = await file.arrayBuffer();
      const fileHash = await hash(fileBuffer);
      let asset = await db.query.mediaAssets.findFirst({ where: eq(mediaAssets.hash, fileHash) });
      if (!asset) {
        console.log(`File with hash ${fileHash} is new. Uploading to R2.`);
        const r2Key = generateUniqueFilename(file.name);
        const bucket = mediaType === "image" ? c.env.IMAGES_BUCKET : c.env.VIDEOS_BUCKET;
        await bucket.put(r2Key, fileBuffer, { httpMetadata: { contentType: file.type } });
        const url = isProd ? `https://${mediaType === "image" ? "images" : "videos"}.chinamedicaltour.vip/${r2Key}` : `/api/media/${mediaType}s/${r2Key}`;
        ...
        console.log(`File with hash ${fileHash} already exists. Skipping R2 upload.`);
      }
      const highestDisplayOrder = await db.select({ value: count() }).from(productMedia).where(eq(productMedia.productId, productId));
      const displayOrder = highestDisplayOrder[0].value;
  ...
  const updatedProduct = await db.query.products.findFirst({
    ...
    with: { media: { with: { asset: true }, orderBy: [productMedia.displayOrder] } }
  });
  return c.json(updatedProduct);
});
app.post("/api/products/:productId/media/reorder", authMiddleware, async (c) => {
  ...
  if (!Array.isArray(mediaIds)) {
    return c.json({ error: "Invalid request body, expected mediaIds array" }, 400);
  }
  const db = drizzle(c.env.DB, { schema: schema_exports });
  try {
    for (let i = 0; i < mediaIds.length; i++) {
      const mediaId = mediaIds[i];
      await db.update(productMedia).set({ displayOrder: i }).where(and(eq(productMedia.id, mediaId), eq(productMedia.productId, productId)));
    }
  } catch (e) {
    console.error("Failed to reorder media:", e);
    return c.json({ error: "Failed to reorder media", details: e.message }, 500);
  }
  const updatedProduct = await db.query.products.findFirst({
    where: eq(products.id, productId),
    with: { media: { with: { asset: true }, orderBy: [productMedia.displayOrder] } }
...
app.post("/api/auth/register", async (c) => {
  ...
  if (!validation.success) {
    return c.json({ error: "Invalid registration data", issues: validation.error.issues }, 400);
  }
  ...
  if (!newUser || newUser.length === 0) {
    return c.json({ error: "Database failed to create user." }, 500);
  }
...
app.post("/api/auth/login", async (c) => {
  ...
  if (!validation.success) {
    return c.json({ error: "Invalid login data", issues: validation.error.issues }, 400);
  }
  const { email, password } = validation.data;
  const db = drizzle(c.env.DB, { schema: schema_exports });
  ...
  const response = {
    ...
    items: cart.items.map((item) => ({
      ...
      product: {
        ...
        imageUrl: item.product.media[0]?.asset.url || null
      }
    }))
  };
...
app.post("/api/cart/items", authMiddleware, async (c) => {
  ...
  await db.update(carts).set({ updatedAt: /* @__PURE__ */ new Date() }).where(eq(carts.id, cart.id));
...
app.put("/api/cart/items/:itemId", authMiddleware, async (c) => {
  ...
  await db.update(cartItems).set({ quantity }).where(and(eq(cartItems.id, itemId), eq(cartItems.cartId, cart.id)));
  await db.update(carts).set({ updatedAt: /* @__PURE__ */ new Date() }).where(eq(carts.id, cart.id));
...
app.get("/api/docs", async (c) => {
  const swaggerHtml = `
    ...
    <body>
        ...
        <script src="https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/5.17.1/swagger-ui-bundle.min.js"><\/script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/swagger-ui/5.17.1/swagger-ui-standalone-preset.min.js"><\/script>
        <script>
            window.onload = function() {
                const ui = SwaggerUIBundle({
                    url: "/openapi.json", // Point to your OpenAPI spec file served by the Worker
                    dom_id: '#swagger-ui',
                    deepLinking: true,
                    presets: [
...
// .wrangler/tmp/bundle-hTJNKW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hTJNKW/middleware-loader.entry.ts
